...
- #BUILD-INFO: 编译相关的参数
  ...
  - OTA_LENGTH: 默认为0, 必须4字节对齐, 当这个值设置时, 编译器会在程序开头填入对应长度的0.(第一个指令是jmp OTA_LENGTH).
  - OTA_FILENAME: OTA程序文件名. 默认为空, 当被设置是, 编译完成后编译器会在当前目录下查找对应文件, 用二进制加载, 并从0地址开始覆盖原程序. 这个文件的大小必须小于设置的`OTA_LENGTH`
...
如果需要将内存以二进制形式保存成文件, 可以用下面的命令
```
dumpbin [地址] [长度] [文件名] // 读取的内容按照二进制写入文件
```
- 写任意位置的内存命令
```
[ww|ws|wb|wf] [地址] [值]
...
```
**注意: 读写的[地址]需要是4的倍数**  
[地址]可以是数字, 也可以是代码中的`全局变量`(用`@export`导出的标签)  
